...
const sections = {
  ...
  allocations: { el: document.getElementById('section-allocations'), nav: document.getElementById('navAllocations'), title: 'Allocations', sub: 'Assign users to consultants' },
};
...
const loadAll = async () => {
  ...
  try {
    ...
    // Allocations load failure is non-critical
  }
};
...
const loadAllocations = async () => {
  ...
  const userSelect = document.getElementById('allocateUser');
  if (userSelect) {
    ...
    if (allUsers.length === 0) {
      ...
      opt.disabled = true;
      opt.textContent = 'No users registered yet';
      userSelect.appendChild(opt);
    } else {
      allUsers.forEach((u) => {
        const opt = document.createElement('option');
        opt.value = u.id;
        opt.textContent = `${getName(u)} (${u.email})${u.status === 'pending' ? ' - [Pending]' : ''}`;
        userSelect.appendChild(opt);
      });
    }
  ...
  const consultantSelect = document.getElementById('allocateConsultant');
  if (consultantSelect) {
    ...
    if (allConsultants.length === 0) {
      const opt = document.createElement('option');
      opt.disabled = true;
      opt.textContent = 'No consultants registered yet';
      consultantSelect.appendChild(opt);
    } else {
      allConsultants.forEach((c) => {
        const opt = document.createElement('option');
        opt.value = c.id;
        opt.textContent = `${getName(c)} (${c.email})${c.status === 'pending' ? ' - [Pending]' : ''}`;
        consultantSelect.appendChild(opt);
      });
    }
    consultantSelect.value = current;
  }

  // Build assignments table
  setHtml('assignmentsList', buildAssignmentsTable(assignments));

  // Bind unassign buttons
  document.querySelectorAll('.unassign-btn').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      const uid = e.currentTarget.dataset.userId;
      const cid = e.currentTarget.dataset.consultantId;
      await unassignUser(uid, cid, e.currentTarget);
    });
  });
};

const buildAssignmentsTable = (assignments) => {
  if (!assignments.length) return '<p class="empty-msg">No assignments yet. Use the form above to assign users to consultants.</p>';
  const rows = assignments.map((a) => `
    <tr>
      ...
      <td>
        <button class="unassign-btn reject-btn" data-user-id="${a.user_id}" data-consultant-id="${a.consultant_id}">✕ Unassign</button>
      </td>
    </tr>
  `).join('');
  return `
...
const unassignUser = async (userId, consultantId, btn) => {
  if (btn) btn.disabled = true;
  try {
    await apiFetch(`/admin/unassign/${userId}/${consultantId}`, { method: 'DELETE' });
