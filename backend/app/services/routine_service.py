...
the user's skin profile and identified concerns.

This is the baseline "expert system" layer described in the architecture
...
def _base_step(
    ...
    """
    Create a standard routine step.
    """
    return {
        "step": order,
        "category": category,
...
def generate_morning_routine(
    ...
    """
    Generate a personalized morning skincare routine.
    """
    ...
    steps = [
        _base_step(
            1,
            "Cleansing",
            "Gentle cleanse to remove overnight buildup.",
        ),
        _base_step(
            2,
            "Toner",
    ...
    steps.append(
        _base_step(
            order + 1,
            "Sun Protection",
            "Apply SPF 30+ broad spectrum sunscreen. Non-negotiable.",
        )
    )
...
def generate_evening_routine(
    ...
    """
    Generate a personalized evening skincare routine.
    """
    ...
    steps = [
        _base_step(
            1,
            "Cleansing",
            "Double cleanse to remove sunscreen/makeup/pollutants.",
        )
    ]
    ...
    steps.append(
        _base_step(
            order,
            "Night Care",
            "Apply night moisturizer/repair cream.",
        )
    )
...
def generate_weekly_treatments(
    ...
    """
    Generate weekly skincare treatments based on concerns.
    """
        ...
        treatments.append(
            {
                "day": "Tue/Fri",
                "treatment": "Clay mask",
                "purpose": "Oil & congestion control",
            }
        )
    ...
    if "dry_skin" in concerns or "sensitive_skin" in concerns:
        treatments.append(
            {
                "day": "Wed",
                "treatment": "Hydrating sheet mask",
    ...
    if "uneven_skin_tone" in concerns or "dark_spots" in concerns:
        treatments.append(
            {
                "day": "Sat",
                "treatment": "AHA/BHA exfoliation",
                "purpose": "Cell turnover, brightening",
            }
        )
...
def apply_seasonal_adjustments(
    ...
    """
    Modify morning/evening/weekly routines based on
    season and environmental exposure.
    """
    ...
    if season == "winter":
        ...
        for step in morning + evening:
            if step["category"] == "Moisturizing":
                step["instruction"] += (
                    " Winter note: switch to a richer, more emollient "
                    "moisturizer -- indoor heating and cold air strip "
                    "moisture faster this time of year."
                )

        for treatment in weekly:
            if "exfoli" in treatment["treatment"].lower():
                treatment["purpose"] += (
                    " (reduced to once weekly in winter "
    ...
    # Summer adjustments
        ...
        for step in morning:
            if step["category"] == "Sun Protection":
                step["instruction"] += (
                    " Summer note: reapply every 2 hours if outdoors; "
                    "consider SPF 50+."
                )
    ...
    if environmental_exposure == "high":
        ...
        for step in morning:
            if step["category"] == "Sun Protection":
                step["instruction"] += (
                    " High environmental exposure detected: "
                    "prioritize reapplication and pair with an "
                    "antioxidant serum (Vitamin C) to counter "
                    "pollution damage."
                )
...
def generate_full_routine(
    ...
    """
    Generate the complete personalized skincare routine.
    """
