...
@router.get("/api/products/recommendations", status_code=status.HTTP_200_OK)
async def get_product_recommendations(
    ...
    try:
        ...
        # ⚠️ AI ENGINEER FIX: Strictly intercept empty assessments! Do not allow generation of dummy products!
        if not assessment:
            return {
                "status": "no_assessment",
                "message": "No skin assessment found. Please complete an assessment first.",
                "data": None
            }
...
# ⚠️ VERCEL 404 FIX: Added redundant routing paths to guarantee interception
@router.get("/image", status_code=status.HTTP_200_OK)
@router.get("/api/products/image", status_code=status.HTTP_200_OK)
