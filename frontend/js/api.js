...
async function ensureSupabaseClient() {
  if (_supabaseInitAttempted) return supabase;
  _supabaseInitAttempted = true;
  if (!supabaseUrl || !supabaseAnonKey) {
    console.warn('Supabase not configured. Running in demo-only mode.');
    return null;
  }
  try {
    ...
    console.warn('Unable to load Supabase dynamically:', err);
    return null;
  }
}
...
export const authAPI = {
  async register(name, email, password) {
    // If Supabase not configured, use demo register immediately
    const client = await ensureSupabaseClient();
    if (!client) return demoRegister(name, email, password);
  ...
  async login(email, password) {
    const client = await ensureSupabaseClient();
    if (!client) return demoLogin(email, password);
  ...
  async getCurrentProfile() {
    ...
    const { data } = await client.from('profiles').select('*').eq('id', current.user.id).maybeSingle();
    return data;
  },
