...
export default function LandingPage({ onSelectRole, onOpenQuiz, onOpenAuth }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      <section style={{ 
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '50px' }}>
            <button className="btn-primary" onClick={onOpenAuth} style={{ fontSize: '1.05rem', padding: '14px 32px' }}>
              <Lock size={18} /> Sign In / Demo Login
            </button>
            <button className="btn-secondary" onClick={onOpenQuiz} style={{ fontSize: '1.05rem', padding: '14px 28px' }}>
          ...
          <div className="glass-card" style={{ 
            ...
            <div>
              <div style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--accent-cyan)' }}>JWT / OAuth2</div>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Role-Based Access Control</div>
            </div>
            <div>
      ...
      {/* AUTHENTICATION DEMO PROMINENT CARD */}
      <section style={{ padding: '40px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        <div className="glass-card glow-animation" style={{ padding: '36px', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '24px' }}>
          <div>
            <span className="badge badge-purple" style={{ marginBottom: '8px' }}>Authentication Module</span>
            <h3 style={{ fontSize: '1.6rem', fontWeight: 700 }}>Demonstrate Role-Based Authentication</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.92rem', marginTop: '4px', maxWidth: '600px' }}>
              Simulates JWT Token bearer auth and OAuth2 login workflows for User, Consultant, Dermatologist, and Administrator roles.
            </p>
          </div>
          <button className="btn-primary" onClick={onOpenAuth} style={{ padding: '12px 28px', fontSize: '1rem' }}>
            <Lock size={16} /> Launch Demo Login Modal
          </button>
        </div>
      </section>
