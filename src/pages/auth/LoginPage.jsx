...
export default function LoginPage() {
    ...
    if (!authSuccess) {
      ...
      setErrorMessage(
        lastError?.message?.includes('Failed to fetch') || lastError?.message?.includes('NetworkError')
          ...
          : lastError?.message || 'Google OAuth Sign-In failed.'
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const targetEndpoints = ['/token', `${API_BASE_URL}/token`];
    let lastError = null;
    let authSuccess = false;
      ...
      <GlassCard glow className="max-w-xl w-full p-8 sm:p-10 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-xs sm:text-sm text-slate-300 leading-relaxed max-w-md mx-auto">
            Authenticates against PostgreSQL database using JWT access tokens & Google OAuth2.
          </p>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder="••••••••••••"
                className="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-3 pr-11 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all"
              />
              <button
                type="button"
